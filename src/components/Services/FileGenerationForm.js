      ...
      <Panel bsStyle='warning'>
        <Form horizontal onSubmit={this.generateFile}>
          <Field name='reportType' component={SelectField} label='Choose type of report' >
            <option value={reportType.DOCX}>{'DOCX agents report'}</option>
            <option value={reportType.XLSX}>{'XLSX agents report'}</option>
            <option value={reportType.CHART}>{'Citations chart'}</option>
          </Field>
          {reportTypeChosen === reportType.CHART
            ? <Field name='createGraph' component={Checkbox} label='I want to create my own graph by selecting agents' />
            : <div>
              ...
              <Field name='takeFirstX' component={TextField} label='Number of agents' />
              <Field name='includeProps' component={ToggleCheckboxButtonGroup} valueChosen={includePropsChosen} label='Include'>
                <ToggleButton value='agentType'>{'Agent type'}</ToggleButton>
