...
import {MAX_ROW_CELLS_COUNT} from '../../constants/agentsTable'
...
export const pickData = R.map(
  R.compose(
    ...
    obj => R.assoc('id', obj.bioagentsID, obj)
  )
)
...
export const updatedData = agents => {
  if (agents.length === 0) {
    return Rx.Observable.of([])
  }
...
export const uptimeData = agents => {
  if (agents.length === 0) {
    return Rx.Observable.of([])
  }

  return agents.map(agent => {
    const {bioagentsID} = agent
    if (!bioagentsID) {
      return Rx.Observable.of(R.compose(
        R.assoc('uptime', []),
      )(agent))
    ...
    return Rx.Observable
      .from(getUptimeInfo(bioagentsID))
      .map(uptimeInfo => {
        return R.compose(
...
export function getServices(query, page = '?page=1') {
  if (query.constructor === Array && query.length > 0) {
    const url = config.getBioAgentsApiUrl(`${page}&${query[0]}`)
    query = query.slice(1)
    return fetch(url)
      .then(response => response.json())
      .then(data => {
        data.list.forEach(agent => {
          agent.publication.forEach(pub => {
  ...
  const url = config.getBioAgentsApiUrl(`${page}&${query}`)
  return fetch(url)
    .then(response => response.json())
    .then(data => {
      data.list.forEach(agent => {
...
export function getChartConfig(citationsYears, agentName, seriesNames) {
  ...
  return {
    title: {
      text: `Citations for ${agentName}`,
    },
    exporting: {
      filename: `chart_${agentName}`,
      buttons: {
        contextButton: {
          text: 'Generate',
          symbolY: 15,
